...
namespace Homework_01
{
    ...
    class Repository
    {
        ...
        /// <summary>
        /// Метод, увольняющий случайных сотрудников, в заданном количестве
        /// </summary>
        /// <param name="value"></param>
        public void DeleteRandomWorker(int value)
        {
            if (value < this.Workers.Count)
            {
                for (int i = 0; i < value; i++)
                {
                    int count = Repository.randomize.Next(this.Workers.Count);
                    this.Workers.RemoveAt(count);
                }
            }
